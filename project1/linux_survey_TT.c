...
asmlinkage void sys_linux_survey_TT(int pid, char __user * result)
{
    ...
    //-----code segment 
   // sprintf(buff, "%08lx-%08lx:", target_mm->start_code, virtophy(target_task->mm,target_mm->start_code));
    //sprintf(buff + strlen(buff), "%08lx-%08lx:", target_mm->end_code, virtophy(target_task->mm,target_mm->end_code));
	//----data segment
    //sprintf(buff + strlen(buff), "%08lx-%08lx:", target_mm->start_data, virtophy(target_task->mm,target_mm->start_data));
	//sprintf(buff + strlen(buff), "%08lx-%08lx:", target_mm->end_data,virtophy(target_task->mm,target_mm->end_data));
	//----heap segment
    //sprintf(buff + strlen(buff), "%08lx-%08lx:", target_mm->start_brk, virtophy(target_task->mm,target_mm->start_brk));
	//sprintf(buff + strlen(buff), "%08lx-%08lx:", target_mm->brk, virtophy(target_task->mm,target_mm->brk));
	//----stack segment
	//sprintf(buff + strlen(buff), "%08lx-%08lx:", target_mm->start_stack,virtophy(target_task->mm,target_mm->start_stack));
    //sprintf(buff + strlen(buff), "%08lx-%08lx:", target_mm->start_stack, virtophy(target_task->mm,target_mm->start_stack));
	//------*****------
	//target_vma->vm_next
